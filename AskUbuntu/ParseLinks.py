...
def parse(post):
    ...
    relatedid = soup.row["relatedpostid"]
    linktype = soup.row["linktypeid"]
    
    return id, date, postid, relatedid, linktype

def worker(queue_in, queue_out):
    while True:
        post = queue_in.get()
        if post == None: break
        id, date, postid, relatedid, linktype = parse(post)
        if id is not None:
            queue_out.put((id, date, postid, relatedid, linktype))
    queue_out.put(None)
