...
def parse(post):
    ...
    voteid = int(soup.row["votetypeid"])
    try:
        date = int(soup.row["creationdate"])
    except:
        date = 0
    return id, postid, voteid, date

def worker(queue_in, queue_out):
    while True:
        post = queue_in.get()
        if post == None: break
        id, postid, voteid, date = parse(post)
        if id is not None:
            queue_out.put((id, postid, voteid, date))
    queue_out.put(None)
