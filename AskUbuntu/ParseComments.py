...
def parse(post):
    ...
    id = int(soup.row["id"])
    postid = int(soup.row["userid"])
    score = int(soup.row["score"])
    ...
    return id,postid,score,text,date,userid

def worker(queue_in, queue_out):
    while True:
        post = queue_in.get()
        if post == None: break
        id,postid,score,text,date,userid = parse(post)
        if id is not None:
            queue_out.put((id,postid,score,text,date,userid))
    queue_out.put(None)
