...
def parse(post):
    ...
    id = int(soup.row["id"])
    tag = soup.row["tagname"]
    count = int(soup.row["count"])
    try:
        postid = int(soup.row["excerptpostid"])
    except:
        postid = 0
    try:
        wikipostid = int(soup.row["wikipostid"])
    except:
        wikipostid = 0
    return id, tag, count, postid, wikipostid

def worker(queue_in, queue_out):
    while True:
        post = queue_in.get()
        if post == None: break
        id, tag, count, postid, wikipostid = parse(post)
        if id is not None:
            queue_out.put((id, tag, count, postid, wikipostid))
    queue_out.put(None)
