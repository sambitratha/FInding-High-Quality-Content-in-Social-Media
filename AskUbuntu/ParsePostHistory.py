...
def parse(post):
    ...
    id = int(soup.row["id"])
    historyid = int(soup.row["posthistorytypeid"])
    postid = int(soup.row["postid"])
    guid = soup.row["revisionguid"]
    date = soup.row["creationdate"]
    userid = int(soup.row["userid"])
    try:
        text = soup.row["text"]
    except:
        say("\rNo text found for post {}; skip.\n".format(id))
        text = " "
    
    return id,historyid,postid,guid,date,userid,text

def worker(queue_in, queue_out):
    while True:
        post = queue_in.get()
        if post == None: break
        id,historyid,postid,guid,date,userid,text = parse(post)
        if id is not None:
            queue_out.put((id,historyid,postid,guid,date,userid,text))
    queue_out.put(None)
